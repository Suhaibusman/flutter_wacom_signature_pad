...
class _PdfSignatureScreenState extends State<PdfSignatureScreen> {
  ...
  /// Convert a rectangle drawn in viewer-local pixels into PDF page
  /// coordinates (points) and persist both the page number and the rect.
  void _savePdfCoords(Rect viewerRect) {
    ...
    const gapPx = 4.0; // fixed inter-page gap in logical pixels

    // Viewer pixels → document pixels (origin = top of entire document)
    ...
    // Walk pages (in PDF-point space) to find which page owns docTop.
    ...
    // Fell past the last page — pin to last page.
  ...
  /// Re-project stored PDF coordinates back to viewer-local pixels so the
  /// overlay rectangle tracks the document correctly on scroll / zoom.
  Rect? _viewerBox() {
    ...
    // Accumulate page heights (in PDF points) above the target page.
    double pageTopPt = 0;
  ...
  Widget _buildPdfArea() {
    ...
    if (_pdfBytes == null) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
            'Enter a PDF URL above and tap Load',
            style: TextStyle(color: Colors.grey.shade600, fontSize: 15),
          ),
          const SizedBox(height: 8),
          Text(
            'Then draw a signature box and sign on the device.',
    ...
    return LayoutBuilder(builder: (context, constraints) {
      // Keep viewer width up-to-date so coordinate mapping stays accurate.
                  ...
                  child: Container(
                    ...
                    child: const Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text(
                          'Click and drag to draw the signature box',
                          style: TextStyle(
                              color: Colors.white,
                              fontSize: 13,
...
/// Dashed blue rectangle drawn while user is dragging to place box.
class _DashedRectPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        rect, Paint()..color = Colors.blue.withValues(alpha: 0.08));
  ...
  @override
  bool shouldRepaint(_DashedRectPainter old) => old.rect != rect;
}

/// Solid border + label shown around the placed signature box.
class _BoxBorderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final corner in [
      ...
      canvas.drawRect(
          Rect.fromCenter(center: corner, width: hs, height: hs), hp);
    }
    // Label
  ...
  @override
  bool shouldRepaint(_BoxBorderPainter old) => old.rect != rect;
}
